...
public:
    ...
    void setEvent(sing::ptr<Event> ev);                     // gets signaled each time a runnable finishes

    bool enqueue(sing::iptr<Runnable> torun);                   // returns false if the queue is full
    sing::iptr<Runnable> getRunnable(bool blocking = false);    // from output queue, returns null if no processed Runnable is present
...
private:
    Event insertion_;
    Event stopped_;
    sing::ptr<Event> def_ready_;
    sing::ptr<Event> ready_;
    ExecuterStatus status_;
    std::vector<sing::iptr<Runnable>> queue_;
    int32_t in_idx_;
    int32_t out_idx_;
    int32_t todo_idx_;
    Atomic done_count_;
...
void run(sing::iptr<Runnable> torun, int32_t stack_size = 0);
int32_t numCores();

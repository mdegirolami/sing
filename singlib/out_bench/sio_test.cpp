...
static bool test_dirfileop()
{
    ...
    if (sing::dirRead("dir1", "*.*", sing::DirFilter::regularOnly, &names, &nfos) != 0) {
        return (false);
    }
    ...
    names.clear();
    if (sing::dirReadNames("dir1", "*.*", sing::DirFilter::dirOnly, &names) != 0) {
        return (false);
    }
    ...
    names.clear();
    if (sing::dirReadNames("dir1", "*.*", sing::DirFilter::listAll, &names) != 0) {
        return (false);
    }
    if (names.size() != 2) {
        return (false);
    }
    if (sing::dirReadNames("dir1", "*.*", sing::DirFilter::listAll, &names, true) != 0) {
        return (false);
    }
    if (names.size() != 3) {
        return (false);
...
static bool test_unicode()
{
    ...
    std::vector<std::string> names;
    std::vector<sing::FileInfo> nfos;
    if (sing::dirRead("その上", "*.*", sing::DirFilter::listAll, &names, &nfos, true) != 0) {
        return (false);
    }
    if (names.size() != 2) {
        return (false);

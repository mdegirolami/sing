...
CaseRange case_ranges[] = {
	...
	{0xA7B4, 0xA7BF, {UpperLower, UpperLower, UpperLower}},
	{0xA7C2, 0xA7C3, {UpperLower, UpperLower, UpperLower}},
...
{
    int	lo = 0;
	int hi = sizeof(case_ranges)/sizeof(case_ranges[0]);
	while (lo < hi) {
		int mm = (hi + lo) >> 1;
		const CaseRange *cr = &case_ranges[mm];
		if (cr->low <= cp && cp <= cr->high) {
            return(cr);
		}
		if (cp < cr->low) {
			hi = mm;
